-- =============================================
-- DEMO SYSTEM RESET - MIGRATION
-- =============================================
-- This migration resets the database to a clean demo-only system
-- WARNING: This will DROP existing tables and data!
-- =============================================

-- Drop existing tables (if they exist)
DROP TABLE IF EXISTS public.cleanup_jobs CASCADE;
...
-- Drop existing views
DROP VIEW IF EXISTS public.active_assistants_view CASCADE;
DROP VIEW IF EXISTS public.user_demo_status CASCADE;

-- Drop existing functions
DROP FUNCTION IF EXISTS public.cleanup_expired_assistants() CASCADE;
DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.update_usage_on_call_end() CASCADE;
DROP FUNCTION IF EXISTS public.check_assistant_expiration() CASCADE;

-- Drop existing triggers
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users CASCADE;
DROP TRIGGER IF EXISTS on_call_log_inserted ON public.call_logs CASCADE;
DROP TRIGGER IF EXISTS on_usage_updated ON public.user_assistants CASCADE;

-- Reset profiles table to demo structure
DROP TABLE IF EXISTS public.profiles CASCADE;

-- =============================================
-- NOW CREATE THE CLEAN DEMO SCHEMA
-- =============================================
...
CREATE POLICY "Users can view own assistants" ON public.user_assistants FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own assistants" ON public.user_assistants FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own assistants" ON public.user_assistants FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete own assistants" ON public.user_assistants FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view questions for own assistants" ON public.structured_questions FOR SELECT USING (assistant_id IN (SELECT id FROM public.user_assistants WHERE user_id = auth.uid()));
CREATE POLICY "Users can insert questions for own assistants" ON public.structured_questions FOR INSERT WITH CHECK (assistant_id IN (SELECT id FROM public.user_assistants WHERE user_id = auth.uid()));
CREATE POLICY "Users can update questions for own assistants" ON public.structured_questions FOR UPDATE USING (assistant_id IN (SELECT id FROM public.user_assistants WHERE user_id = auth.uid()));
CREATE POLICY "Users can delete questions for own assistants" ON public.structured_questions FOR DELETE USING (assistant_id IN (SELECT id FROM public.user_assistants WHERE user_id = auth.uid()));
