-- FINAL COMPREHENSIVE FIX - CORRECTED FOR DATA TYPE COMPATIBILITY
-- This fixes all issues: evaluation constraints, foreign keys, and data type mismatches
...
-- =============================================
-- STEP 3: Fix data type compatibility for foreign key
-- =============================================
...
-- OPTION A: Change assistant_id in call_info_log from UUID to TEXT to match vapi_assistant_id
ALTER TABLE public.call_info_log ALTER COLUMN assistant_id TYPE TEXT USING assistant_id::TEXT;

-- OR OPTION B: Change vapi_assistant_id from TEXT to UUID (if all values are valid UUIDs)
-- ALTER TABLE public.user_assistants ALTER COLUMN vapi_assistant_id TYPE UUID USING vapi_assistant_id::UUID;
...
-- =============================================
-- STEP 5: Update helper function to work with TEXT VAPI IDs
-- =============================================
...
-- =============================================
-- STEP 6: Update usage calculation function to work with TEXT assistant IDs
-- =============================================
...
-- =============================================
-- STEP 7: Recreate trigger with flexible evaluation and TEXT assistant ID support
-- =============================================
...
BEGIN
  -- Get user_id from VAPI assistant ID (now TEXT)
...
-- Recreate trigger with flexible evaluation and TEXT assistant ID handling
...
-- =============================================
-- STEP 8: Update indexes for better performance with TEXT assistant IDs
-- =============================================
-- Drop old index if exists
DROP INDEX IF EXISTS idx_call_logs_duration_evaluation;
DROP INDEX IF EXISTS idx_call_info_log_duration_evaluation;

-- Create new index optimized for TEXT assistant ID queries
...
-- =============================================
-- Check data types are now compatible
SELECT 
...
FROM pg_constraint 
WHERE conrelid = 'call_info_log'::regclass;
...
SELECT 'FINAL FIX COMPLETE - Data types compatible, Make.com can now send VAPI assistant IDs directly' as status;